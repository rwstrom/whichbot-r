//
// $Id: engine_api.cpp,v 1.8 2004/08/11 22:08:59 clamatius Exp $
...
enginefuncs_t meta_engfuncs = 
{
	...
	interceptRemoveEntity,		// pfnRemoveEntity()
	...
	interceptMessageBegin,		// pfnMessageBegin()
	interceptMessageEnd,		// pfnMessageEnd()

	interceptWriteByte,			// pfnWriteByte()
	interceptWriteChar,			// pfnWriteChar()
	interceptWriteShort,		// pfnWriteShort()
	interceptWriteLong,			// pfnWriteLong()
	interceptWriteAngle,		// pfnWriteAngle()
	interceptWriteCoord,		// pfnWriteCoord()
	interceptWriteString,		// pfnWriteString()
	interceptWriteEntity,		// pfnWriteEntity()
	...
	interceptCmdArgs,				// pfnCmd_Args()
	interceptCmdArgv,				// pfnCmd_Argv()
	interceptCmdArgc,				// pfnCmd_Argc()
	...
	interceptGetWonID,			// pfnGetPlayerWONId()
...
C_DLLEXPORT int GetEngineFunctions(enginefuncs_t* pengfuncsFromEngine, int* interfaceVersion) 
{
	if(!pengfuncsFromEngine) {
		UTIL_LogPrintf(const_cast<char*>("GetEngineFunctions called with null pengfuncsFromEngine"));
		return(FALSE);
	}
	else if(*interfaceVersion != ENGINE_INTERFACE_VERSION) {
		UTIL_LogPrintf(const_cast<char*>("GetEngineFunctions version mismatch; requested=%d ours=%d"), *interfaceVersion, ENGINE_INTERFACE_VERSION);
